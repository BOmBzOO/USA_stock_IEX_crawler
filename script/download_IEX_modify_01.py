...
def get_dates():
    ...
    try:
        ...
        last_date = datetime.strptime(min(folder_date)[0:8], '%Y%m%d').date()
        print(last_date)

        # start_date = max(IEX_date, last_date + timedelta(days=1))
        start_date = last_date - timedelta(days=60)  # 30 trailing calendar days acc. to IEX docu
        print(start_date)
    ...
    dates = [start_date + timedelta(days=i) for i in range((last_date - start_date).days)]
    # print(dates)
...
if __name__ == '__main__':
    ...
    init_logging()
    todo_dates = list(reversed(get_dates()))
    # todo_dates = ['20200806','20200807']
    print(todo_dates)
